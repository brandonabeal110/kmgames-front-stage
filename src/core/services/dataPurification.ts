...
import {TLeague, TMatchStatistics as matchStatisticsType, TLeagueStatistic, TGameResultStatistic, TGameResultCommon, TGameFootballDetail, TGameBasketballDetail, TGameTennisDetail, TGameVolleyballDetail, TEGameDetail} from '@core/reducers/_reduxStore.d';
...
export const mapMatchList = (matchList: Array<TMatchDetail>, type: 'match-list' | 'match-detail' | 'favorite-list'): {
  ...
  _.each(matchList, (item: TMatchDetail): void | boolean => {
    ...
    const match: TMatch = {
      ...
      competitionType: (configs.SPORT.COMPETITION_TYPE[item.md.sid] && item.md.me?.spt) ? `${item.md.me?.spt}${configs.SPORT.COMPETITION_TYPE[item.md.sid]}${Math.floor(item.md.me?.spt / 2) + 1}胜` : null,
      playTypes: (item.ptds || []).map((odd: TPlayTypeDetail) => ({
        ...
        mks: odd.hcl.map((market) => ({ // 目前只做一条线
          ...
          ops: (odd.kc === 'WinningMargin' ? JsqSort(market.bil) : market.bil).filter((od) => !!od).map((od: TBetItemLine, odIndex) => ({
            ...
            playName: odd.bn,
            id: Number(od.bid),
            name: od.n || (DX_BALL.includes(odd.kc) ? od.hoa === 'Over' ? `大 ${od.h}` : `小 ${od.h}` : od.h) || od.hoa,
            orderName: od.h,
            betTeam: od.n ? '' : od.hoa || od.h,
            betHandicap: od.h,
            hoa: od.hoa,
            locked: od.s === 4,
            ended: od.s === 5,
...
export const mapGameResultStatistics = (GameResultStatistics: Array<TGameResultStatisticsItem>, sportId: number): Array<TGameResultStatistic> => {
  ...
  const strSplitWithEmpty = (str: string): string[] => {
    ...
    if (Number(splitRes[0]) === 0 && Number(splitRes[1]) === 0) {
      return ['-', '-'];
    };
    return splitRes;
  };
  ...
  const goalAdder = (firstGoal: string, secondGoal: string) => {
    const resArr = strSpliter(firstGoal).map((el, ind) => Number(el) + Number(strSpliter(secondGoal)[ind]));

    if (resArr[0] === 0 && resArr[1] === 0) return ['-', '-'];
    return resArr;
  };
  ...
  const splitFootball = (i: TGameResultStatisticsItem): TGameResultCommon & {details: TGameFootballDetail} => {
    return {
      ...
      details: {
        ...
        // 要返回点球大战，但是现在不支持这个，而利用最总点球
        [EGameResult.PK]: strSplitWithEmpty(i.finalGoal),
  ...
  const splitBasketball = (i: TGameResultStatisticsItem): TGameResultCommon & {details: TGameBasketballDetail} => {
    if (i.periodType === 4) {
      return {
        ...splitCommon(i),
        details: {
          ...
          [EGameResult.Q3G]: strSplitWithEmpty(i.q3Goal),
  ...
  const splitTennis = (i: TGameResultStatisticsItem): TGameResultCommon & {details: TGameTennisDetail} => {
    return {
      ...splitCommon(i),
      details: {
        [EGameResult.Q1G]: strSpliter(i.q1Goal),
  ...
  const splitVolleyball = (i: TGameResultStatisticsItem): TGameResultCommon & {details: TGameVolleyballDetail} => {
    return {
      ...splitCommon(i),
      details: {
        [EGameResult.Q1G]: strSpliter(i.q1Goal),
  ...
  const splitEGame = (i: TGameResultStatisticsItem): TGameResultCommon & {details: TEGameDetail} => {
    return {
      ...splitCommon(i),
      details: {
