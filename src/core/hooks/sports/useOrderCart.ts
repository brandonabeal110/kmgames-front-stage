...
export default () => {
  ...
  const confirmSubmitStatus = useSelector((state: IStore) => state.user.confirmSubmitStatus);
  ...
  const addOrder = React.useCallback((odd: OddType) => {
    const order = _.find(orders, (item) => item.matchId === odd.matchId);
    if (!order && orderTags.length === 10) {
    ...
    const maxBetAmount = getMaxLimitAmount(odd.lv, odd.sportId, odd.isRunning ? 1 : 2, odd.kc);
    dispatch(ACTIONS.SPORT.addOrder({data: {...odd, money: '', currentOddType, sportNameText: sportName, europeOdd: Number(europeOdd), maxBetAmount}}));
    visibleBetOrderMitt.emit({display: true});
  }, [orders, user.currentOddType, maxBalance, isSeries]);
