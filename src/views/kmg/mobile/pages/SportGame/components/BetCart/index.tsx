...
export default () => {
  ...
  const {removeAllOrder, verifyOrderTokenThenSubmitSingleOrder, setConfirmAcceptChangeOdd, submitLoading, isSeries, getSeriesData, confirmSubmitStatus} = useOrderCart();
  ...
  const submit = ()=>{
    if (!isSeries && Number(order.money) < order.minBetAmount) {
