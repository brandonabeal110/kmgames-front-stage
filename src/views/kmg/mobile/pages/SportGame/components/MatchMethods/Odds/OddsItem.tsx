...
export const OddItem = ({op, isNoob, isHome, match, len, isInList, methodCode, inMainList}: IOddItemProps) => {
  ...
  if (!op.available) {
    return (
      isNoob ? <div className={classnames(style.item, 'odd-item', 'disabled', {'is-home': isHome === 0, 'is-draw': len === 3 && isHome === 1})}>
        <div className={classnames('name', {'text-theme': /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(mapBetItemName[op?.name] || op.name)})}>
          <span className="name-text">
            {(isHome === 0 ? op.teams.home.name : (isHome === 1 && len === 3) ? '和' : op.teams.away.name) }
          </span>
          {
        ...
        <LockOutlined />
      </div> : <div className={classnames(style.item, 'disabled')}>
        <LockOutlined />
      </div>
    );
  }
    ...
    <>
      <div className={classnames(style.item, 'odd-item', {'is-home': isHome === 0, 'is-draw': len === 3 && isHome === 1, 'active': _.includes(orderTags, op.tag)})} onClick={() => addAndRemoveOrder(op)}>
        <div className={classnames('name')}>
          {
            isNoob ? <span className="name-text">
              {
                (isHome === 0 ? op.teams.home.name : (isHome === 1 && len === 3) ? '和' : op.teams.away.name)
              }
            </span> : <span className={classnames({'text-theme': !methodCode.endsWith('_AH') && /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(mapBetItemName[op?.name] || op.name)})}>
