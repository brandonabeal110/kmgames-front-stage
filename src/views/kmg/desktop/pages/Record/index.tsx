...
import {chooseEuropeOrAsiaText, displayHomeAwayScore, displayBetItem, getPlayNameByKc, getBetTeamType, isVisiableSecondText, getBetHandiCapAtBetting} from '@core/utils';
  ...
  return (
    <ul className='content' key={data.id}>
          ...
          <div className={`${data.seriesType!==1 ? 'series-item' : ''} group-items`} key={detail.orderId}>
            ...
            <div className='teams'>
              ...
              <span>{detail?.awayTeamNameCn}
                <em className='m8'>{EOrderTypeStatus.UNSETTLED === data.billStatus && <span>{displayHomeAwayScore(detail?.scoreBenchmark, detail.sportId)}</span>}</em>
              </span>
              {data.billStatus === EOrderTypeStatus.SETTLED && <span>{displayHomeAwayScore(detail?.scoreBenchmark, detail.sportId)}</span> }
              <span style={{marginLeft: '8px'}}>{dayjs(detail?.beginTime).format('MM-DD HH:mm')}</span>
            </div>
            {(data.status !== EOrderStatus.AUTO_CANCLLED && data.status !== EOrderStatus.MANUAL_CANCLLED && data.billStatus == EOrderTypeStatus.SETTLED && detail?.sportId !== ESportType.TENNIS) &&
            <div className='settled-score'>{displayBetItem(detail.betItemName, detail.sportId)} {displayHomeAwayScore(detail?.settleScore, detail.sportId)}</div>}
          </div>
        ))}
      </li>
      ...
      <li>
        ...
        >
          {activeTab === EOrderTypeStatus.RESERVER && data.status === 0 ? '预约中' : activeTab === EOrderTypeStatus.UNSETTLED ? ORDER_STATUS_MAP[EOrderTypeStatus.UNSETTLED] : ORDER_STATUS_MAP[data.status]}
