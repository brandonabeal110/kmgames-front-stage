...
export default React.memo(({match, market}: IProps) => {
  ...
  const getOps = (market:PlayType, type:String = 'Home')=>{
    ...
    return ops.find(((item)=>item.type.toString().toUpperCase() === type.toUpperCase() || (type === 'Home' ? item.type.toString().toUpperCase() === item.teams.home.name.toUpperCase() : item.type.toString().toUpperCase() === item.teams.away.name.toUpperCase())));
  };
