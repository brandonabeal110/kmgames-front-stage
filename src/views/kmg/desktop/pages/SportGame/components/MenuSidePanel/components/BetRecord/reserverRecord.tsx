...
const tabs = [
  {
    name: '进行中',
    params: {status: 0, billStatus: 0},
  },
  {
    name: '已取消',
    params: {status: 22, billStatus: 1},
  },
  {
    name: '预约失败',
    params: {status: 21, billStatus: 1},
  },
];
export default React.memo(({betType}: IProps) => {
  const searchParams = {billStatus: 0, pageNum: 1, status: 0, isReserve: 1};
  ...
  React.useEffect(()=>{
    const searchParams = {billStatus: 0, pageNum: 1, status: reserveType};
        ...
        <div className={classnames('reserve-tab')} >
          {
            tabs.map((item)=>{
              return (
                <div className={classnames(reserveType === item.params.status ? 'active' : '')}
                  key={item.name}
                  onClick={()=>{
                    setReserveType(item.params.status);
      ...
      <div className='order-list'>
        {
          ...
          list.filter((data) => data.status === reserveType).map((item, idx: number) => <OrderItem showConfirm={showConfirm} data={item} key={idx} betType={betType}/>)}
