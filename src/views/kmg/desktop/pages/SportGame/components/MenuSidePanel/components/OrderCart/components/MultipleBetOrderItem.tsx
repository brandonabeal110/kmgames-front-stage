...
const OrderItem = React.memo(({data, showOdd, inputRef, multi = true, seriesList, setActiveFastBtn, activeFastBtn}: IProps) => {
  ...
  return (
    <div className={classnames('bet-item')} key={data.id}
      ...
      <div className='input-box'>
        <input placeholder={isOverBetAmount(data.minBetAmount, data.maxBetAmount).text} type='text' disabled={isOverBetAmount(data.minBetAmount, data.maxBetAmount).flag?true: false}
          ref={(ref) => {
      ...
      {
        data.overAmountLimit && <div className="error-message"> {`有效投注金额范围为 ${data.minBetAmount} - ${data.maxBetAmount}。`}</div>
      }
      {
