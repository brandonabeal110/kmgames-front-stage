...
export default React.memo(({hideOrderCart}: IProps) => {
  ...
  const {removeAllOrder, removeOrder, orders, orderTags, isExistConfirmed, verifyOrderTokenThenSubmitSingleOrder,
    confirmSubmitStatus, setConfirmAcceptChangeOdd, submitLoading, seriesList, getSeriesData} = useOrderCart();
  ...
  const hasSameMatchOrder = () => {
    const flag = orderStatus.sameMatch.length > 0;
    return flag && (
  ...
  const renderErrorMsg =()=>{
    return (
      <button className='fail-staus od-status-text'>
        <p>{alert}</p>
      </button>
    );
  };
        ...
        <>
          ...
          <div className='bottom-content'>
            <div className='multiple-bottom-wrap'>
              ...
              {
                !alert && hasTwoOrderMore() && hasTransformOrder() && !hasInvalidOrder() && <div className='od-status-text'>赔率已更改</div>
              }
              {/* 移除失效的注单 */}
              ...
              <div className="action">
                ...
                {
                  // 正常
                  !assertSameMatchOrder()&& !hasTransformOrder() && !hasInvalidOrder() &&
                ...
                {
                  // 失效
                  (assertSameMatchOrder() || hasInvalidOrder()) &&
